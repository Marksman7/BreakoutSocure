...
public class Util : MonoBehaviour
{
    ...
    public static Vector3 BounceOffWallForPaddle(Vector3 position, float width, float height, ref Vector2 direction, bool Leftside)
    {
        //if(cameraRect.xMin == cameraRect.x) throw new UnityException("No instance of Util in Scene");
        if (!cameraRect.Contains(position))
        {
            ...
            {
                if (position.x <= (cameraRect.xMax/2))
                {
                    if (showDebug) Debug.Log(string.Format("left xMin {0} pos {1} w {2} h {3} direction {4}", cameraRect.xMin, position, width, height, direction));
                    direction.x *= -1;
                }
            ...
            if (Leftside == true)
            {
                position.x = Mathf.Clamp(position.x, cameraRect.xMin, (cameraRect.xMax/2));
            }
            else if (Leftside == false)
            {
                position.x = Mathf.Clamp(position.x, (cameraRect.xMax/2), cameraRect.xMax);
            }
